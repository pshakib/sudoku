        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if(event.key == 48):
                    ...
                    pygame.draw.rect(screen, color1, (position[0]*50 + buffer, position[1]*50 + buffer, 50 - buffer, 50 - buffer))
...
def main():
    ...
    pygame.display.set_caption("Sudoku by Shakib")
    screen.fill(color1)
    myfont = pygame.font.SysFont('Comic Sans MS', 35)
    ...
    for i in range(0, len(grid[0])):
        for j in range(0, len(grid[0])):
            if(0<grid[i][j]<10):
                value = myfont.render(str(grid[i][j]), True, color2)
